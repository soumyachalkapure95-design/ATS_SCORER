...
def sign_in_with_password(email: str, password: str) -> Dict[str, Any]:
    err = _missing_config()
    if err:
        logger.info('Supabase not configured. Falling back to local developer mode.')
        return {
            'access_token':  'mock_access_token',
            'refresh_token': 'mock_refresh_token',
            'user_id':       'local_user_id',
            'email':         email or 'developer@local.host',
        }
    try:
...
def google_oauth_url() -> Dict[str, Any]:
    err = _missing_config()
    if err:
        return {'error': 'Local developer mode is active (Supabase not configured)'}
    try:
